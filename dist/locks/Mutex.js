"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var Mutex = /** @class */ (function () {
    function Mutex(syncedNotifier) {
        this.syncedNotifier = syncedNotifier;
        this.lockAcquired = false;
        this.lock = this.lock.bind(this);
        this.unlock = this.unlock.bind(this);
        this.asyncLock = this.asyncLock.bind(this);
    }
    Mutex.once = function (mutex, resource, onceFN) {
        if (resource.load() === 1) {
            return;
        }
        mutex.lock();
        // maybe someone already flagged it
        if (resource.load() === 1) {
            mutex.unlock();
            return;
        }
        onceFN(function () {
            resource.store(1);
            mutex.unlock();
        });
    };
    Mutex.prototype.lock = function () {
        this._throwIfLocked();
        while (true) {
            // lock is already acquired, wait
            if (this.syncedNotifier.load() > 0) {
                this.syncedNotifier.waitEqualZero();
            }
            var countOfAcquiresBeforeMe = this.syncedNotifier.incrementOne();
            // someone was faster than me, try again later
            if (countOfAcquiresBeforeMe >= 1) {
                this.syncedNotifier.decrementOne();
                continue;
            }
            this.lockAcquired = true;
            return;
        }
    };
    Mutex.prototype.asyncLock = function (interval, cb) {
        var _this = this;
        if (interval === void 0) { interval = 100; }
        if (cb !== undefined) {
            this._asyncLock(interval, cb);
            return;
        }
        return new Promise(function (resolve) {
            _this._asyncLock(interval, resolve);
        });
    };
    Mutex.prototype.unlock = function () {
        if (!this.lockAcquired) {
            throw new Error('mutex is not acquired, locking nothing');
        }
        this.syncedNotifier.decrementOne();
        this.syncedNotifier.notify(1);
        this.lockAcquired = false;
    };
    Mutex.prototype.runPromiseInLock = function (p) {
        return __awaiter(this, void 0, void 0, function () {
            var res;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        this.lock();
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, , 3, 4]);
                        return [4 /*yield*/, p];
                    case 2:
                        res = _a.sent();
                        return [2 /*return*/, res];
                    case 3:
                        this.unlock();
                        return [7 /*endfinally*/];
                    case 4: return [2 /*return*/];
                }
            });
        });
    };
    Mutex.prototype._throwIfLocked = function () {
        if (this.lockAcquired) {
            throw new Error('locked already acquired');
            return;
        }
    };
    Mutex.prototype._asyncLock = function (interval, cb) {
        if (this.syncedNotifier.load() > 0) {
            setTimeout(this.asyncLock, interval);
            return;
        }
        var countOfAcquiresBeforeMe = this.syncedNotifier.incrementOne();
        // someone was faster than me, try again later
        if (countOfAcquiresBeforeMe >= 1) {
            this.syncedNotifier.decrementOne();
            setTimeout(this.asyncLock, interval);
            return;
        }
        this.lockAcquired = true;
        cb();
    };
    return Mutex;
}());
exports.default = Mutex;
